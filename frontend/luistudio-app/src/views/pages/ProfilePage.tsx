...
type ProfileTab = 'profile' | 'activity' | 'sessions'
...
export function ProfilePage({ user, sessions, sessionsLoading, activity, activityLoading, activityFrom, activityTo, activityPage, activityTotalPages, onActivityFromChange, onActivityToChange, onActivityPageChange, onClearActivityFilters, onRevokeSession, onRevokeAllSessions }: ProfilePageProps) {
        ...
        <article className="card account-card">
          ...
          {activeTab === 'profile' && <section className="account-section" aria-labelledby="personal-data-title">
            ...
            <div className="account-details">
              <ProfileField label="Código" value={user.code} /><ProfileField label="Correo" value={user.email} /><ProfileField label="Nombres" value={user.firstName} /><ProfileField label="Apellidos" value={user.lastName} />
          ...
          {activeTab === 'activity' && <section className="account-section" aria-labelledby="activity-title">
            ...
            <FilterBar
              filters={[
                { id: 'activity-from-filter', type: 'date', value: activityFrom, onChange: onActivityFromChange, ariaLabel: 'Actividad desde fecha' },
                { id: 'activity-to-filter', type: 'date', value: activityTo, onChange: onActivityToChange, ariaLabel: 'Actividad hasta fecha' },
              ]}
              quickChips={[]}
              actions={<button type="button" className="secondary-btn" onClick={onClearActivityFilters} disabled={!activityFrom && !activityTo}>Reiniciar filtros</button>}
            />
            {activityLoading ? <EmptyCopy text="Cargando actividad…" /> : activity.length === 0 ? <EmptyCopy text="Sin actividad registrada para este periodo." /> : <div className="account-list">{activity.map((item) => <article className="account-list-item" key={item.id}><i className={`account-event-dot ${ACTION_CLASSES[item.action] ?? 'neutral'}`} /><div><strong>{ACTION_LABELS[item.action] ?? item.action}</strong><div className="account-event-meta">{activityMetadata(item.detail).map((meta) => <span key={`${meta.label}-${meta.value}`}><b>{meta.label}:</b> {meta.value}</span>)}</div><time>{formatDate(item.createdAt)}</time></div></article>)}</div>}
            {!activityLoading && activity.length > 0 && <Pagination page={activityPage} totalPages={activityTotalPages} onPrev={() => onActivityPageChange(Math.max(1, activityPage - 1))} onNext={() => onActivityPageChange(Math.min(activityTotalPages, activityPage + 1))} />}
          </section>}
