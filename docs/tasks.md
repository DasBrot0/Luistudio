...
Este archivo conserva el desglose de trabajo por historia de usuario. Las filas expresan las tareas que dieron origen a la implementación, aunque mantengan verbos como “crear” o “agregar”.

El Release 1 contiene las historias oficiales registradas en la documentación del curso. Las tareas de Release 2 están alineadas con el estado implementado; para criterios finales y alcance vigente prevalece [release-02.md](release-02.md).

`tasks-by-epica.md` debe eliminarse o reemplazarse por una referencia a este documento para evitar duplicidad.
...
El Release 2 se distribuye en tres sprints para evitar concentrar mapa, notificaciones, dashboard e IA en una sola iteración.

| Sprint   | Enfoque                               | Historias principales                                             |
| -------- | ------------------------------------- | ----------------------------------------------------------------- |
| Sprint 5 | Seguridad avanzada y gestión de cuenta | E2-H2, E2-H7, E4-H6, E4-H7, E4-H9, E4-H10                         |
| Sprint 6 | Disponibilidad y notificaciones       | E1-H10, E3-H13, E3-H4, E3-H9, E3-H10                              |
| Sprint 7 | Analítica e inteligencia              | E1-H12, E5-H1                                                      |
...
| Front-End     | Permitir actualización manual del mapa; no realizar polling automático.                                                                                     |
...
| Pruebas       | La acción manual de actualización vuelve a consultar los estados.                                                                                         |
...
**Sprint:** 7 · **Prioridad:** Media · **Tamaño:** L
...
| Área          | Tarea                                                                                                                                              |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Front-End      | Agregar una vista de búsqueda inteligente con texto libre y hasta 3 recomendaciones.                                                               |
...
| Front-End      | Agregar método `intelligentRoomSearch` en `frontend/luistudio-app/src/services/api.ts`.                                                             |
| Front-End      | Permitir seleccionar una sala recomendada para precargar el formulario de reserva.                                                                  |
| Back-End       | Crear endpoint `POST /api/rooms/intelligent-search`.                                                                                               |
| Back-End       | Crear DTOs `IntelligentRoomSearchRequest`, `IntelligentRoomSearchResponse` y `RoomSearchIntent`.                                                   |
| Back-End       | Usar `GroqRoomIntentInterpreter` para obtener intención estructurada y fallback local si Groq falla.                                                |
| Back-End       | Usar `IntelligentRoomSearchService` para filtrar, puntuar y ordenar de forma determinista.                                                         |
...
| Base de datos | Evitar más de una suscripción `ACTIVA` o `EN_COLA` del mismo usuario para la misma sala.                                                                        |
...
| Back-End      | Pasar la suscripción a `EN_COLA` y marcarla `NOTIFICADA` únicamente después del envío exitoso.               |
...
| Back-End      | Capturar IP reenviada por el proxy y user-agent en login, y derivar una etiqueta estable de dispositivo.          |
...
| Back-End      | Ajustar `POST /api/auth/logout` para revocar el JTI, registrar el cierre y limpiar la cookie.                                                              |
...
| Back-End      | Aplicar confirmación por correo únicamente a la desactivación de 2FA. El cambio de contraseña no pertenece a este flujo y las sesiones se revocan directamente. |
...
| Área         | Tarea                                                                                                                |
| ------------- | -------------------------------------------------------------------------------------------------------------------- |
| Front-End     | Crear página de dashboard administrativo.                                                                            |
| Front-End     | Mostrar tarjetas o gráficos de tasa de ocupación, horas pico, estudiantes con más reservas y tasa de inasistencia.  |
| Front-End     | Agregar rango de fechas, restablecimiento, última actualización y exportación CSV.                                    |
| Back-End      | Crear un único endpoint `GET /api/admin/dashboard?from=YYYY-MM-DD&to=YYYY-MM-DD`.                                      |
| Back-End      | Incluir ocupación diaria, mapa de calor, asistencia, ocupación por sala y ranking en la respuesta.                    |
| Back-End      | Calcular tasa de ocupación con base en reservas activas/completadas y disponibilidad de salas.                       |
| Base de datos | Reutilizar `bookings`, `rooms`, `users` e historial de inasistencias.                                                |
| Base de datos | Opcional: crear vista o consulta agregada para métricas por día.                                                     |
| Pruebas       | Validar cálculos con datos conocidos.                                                                                |
| Pruebas       | Verificar filtros por fecha.                                                                                         |
| Pruebas       | Restringir acceso solo a administrador.                                                                              |
